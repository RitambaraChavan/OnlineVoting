...
public class UserController {
	...
	public ResponseEntity<User> updateUser(@RequestBody User user) {
	  ...
	  @GetMapping("/findByVotingCardNumber/{userVotingCardNumber}") public
	  List<User> getUserByVotingCardNumber(@PathVariable String
	  userVotingCardNumber) { return
	 userService.getUserByVotingCardNumber(userVotingCardNumber); }
	  
	  @GetMapping("/findByUserId/{userId}") public User
	  getUserByVotingCardNumber(@PathVariable Long userId) { return
	  uRepository.findById(userId).get(); }
	  
	  @GetMapping("/findByMobileNumber/{userMobileNumber}") public List<User>
	  getUserByMobileNumber(@PathVariable String userMobileNumber) { return
	  userService.getUserByMobileNumber(userMobileNumber); }
	  
	  @GetMapping("/findByUserName/{userName}") public List<User>
	  getUserByUserName(@PathVariable String userName) { return
	  userService.getUserByUserName(userName); }
	  
	  @GetMapping("/findByRole/{userRole}") public List<User>
	  getUserByRole(@PathVariable String userRole) { return
	  userService.getUserByRole(userRole); }
	  
	  @PostMapping("/activateUser") public ResponseEntity<String>
	  activateUser(@Valid @RequestBody User user) { user.setActivateAccount(true);
	  uRepository.save(user); return
